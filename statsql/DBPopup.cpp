...
DBPopup::DBPopup(std::shared_ptr<mysqlx::Session> temp_session, std::function<void()> cb, const wxString& title) : wxFrame(nullptr, wxID_ANY, title), _s(temp_session), callback(cb) {
...
void DBPopup::HandleCreateDBClick(wxMouseEvent& evt) {
	_s->createSchema(static_cast<std::string>(dbname->GetValue()));
	this->callback();
	this->Destroy();
}
