...
MainFrame::MainFrame(std::shared_ptr<mysqlx::Session> temp_session) : wxFrame(nullptr, wxID_ANY, "Statsql"), m_session(std::move(temp_session)) {
...
#ifdef DEBUG
		wxLogStatus("swapped to structure");
#endif // DEBUG
	}
}
...
#ifdef DEBUG
		wxLogStatus("swapped to data");
#endif // DEBUG
	}
}

void MainFrame::HandleAddRowToResultsDataListbox(wxMouseEvent& evt) {
	wxVector<wxVariant> y;
	//for (const auto& z : m_types_map) {
	//}
	//y.push_back(true);
	//results_data_listbox->AppendItem(y);
	m_types_map;
}
...
void MainFrame::HandleResultsEditEnd(wxDataViewEvent& evt) {
	...
	wxLogStatus(col->GetTitle());
	wxLogStatus("Edited from: %s to %s", m_editing_start, v.GetString());
}

void MainFrame::OnDeleteRowClicked(wxCommandEvent& evt) {
	wxLogStatus("deleted row");
}

void MainFrame::OnAddRowClicked(wxCommandEvent& evt) {
	wxLogStatus("added row");
}
...
void MainFrame::HandleResultsContextMenu(wxDataViewEvent& evt) {
	...
	if (item.IsOk()) {
		...
		a << row << " " << col << " item: " << m_results_structure_rclick_item.GetString();
...
void MainFrame::OnDropDBClicked(wxCommandEvent& evt) {
	...
	wxMessageDialog* dia = new wxMessageDialog(this, "Drop database '" + to_drop + "' ?", "Statsql", wxYES_NO);
	int a = dia->ShowModal();
	if (a == wxID_YES) {
...
void MainFrame::OnDropTableClicked(wxCommandEvent& evt) {
	...
	wxMessageDialog* dia = new wxMessageDialog(this, "Drop table '" + to_drop + "' ?", "Statsql", wxYES_NO);
	int a = dia->ShowModal();
	if (a == wxID_YES) {
