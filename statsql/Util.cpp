...
namespace Util {
	std::vector<uint64_t> Util::mysqlx_raw_as_u64_vector(const mysqlx::Value& in_value) {
	...
	std::chrono::year_month_day Util::read_date(const mysqlx::Value& value) {
		const auto vector = Util::mysqlx_raw_as_u64_vector(value);
		if (vector.size() < 3)
	...
	std::chrono::system_clock::time_point Util::read_date_time(const mysqlx::Value& value) {
		const auto vector = Util::mysqlx_raw_as_u64_vector(value);
		if (vector.size() < 3)
